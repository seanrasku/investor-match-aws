...
export const createTag = async (token, userId, name) => {
  try {
    return await axios.post(
      `${process.env.REACT_APP_API}/tag/${userId}`,
      name,
      {
        headers: {
...
export const addTag = async (token, userId, name, tagId) => {
  try {
    return await axios.post(
      `${process.env.REACT_APP_API}/user/${userId}/${tagId}`,
      name,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const updateTag = async (token, data, userId, tagId) => {
  ...
  try {
    return await axios.patch(
      `${process.env.REACT_APP_API}/tag/${userId}/${tagId}`,
      { name: data },
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const adminDeleteTag = async (token, userId, tagId) => {
  try {
